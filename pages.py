...
/* ══════ صندوق‌های اطلاعات سفارشی ══════ */
.info-boxes{display:grid;grid-template-columns:repeat(2,1fr);gap:12px;margin-bottom:16px}
.info-box{background:var(--card);border:1px solid var(--card-b);border-radius:14px;padding:16px;display:flex;flex-direction:column;gap:8px;transition:all .2s ease}
.info-box:hover{border-color:var(--card-bh);box-shadow:0 4px 12px rgba(0,0,0,.12)}
.info-box-icon{width:36px;height:36px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:18px}
.info-box-icon.time{background:rgba(59,130,246,.15);color:var(--accent2)}
.info-box-icon.users{background:rgba(139,92,246,.15);color:var(--purple)}
.info-box-label{font-size:10px;color:var(--t3);font-weight:700;text-transform:uppercase;letter-spacing:.05em}
.info-box-value{font-size:18px;font-weight:800;color:var(--t1)}
.info-box-sub{font-size:9px;color:var(--t3)}

.volume-warning{background:linear-gradient(135deg,rgba(217,119,6,.12),transparent);border:1px solid rgba(217,119,6,.25);border-radius:12px;padding:12px 14px;margin-bottom:14px;display:flex;gap:10px;align-items:flex-start}
.volume-warning-icon{width:32px;height:32px;border-radius:8px;background:rgba(217,119,6,.2);color:#d97706;display:flex;align-items:center;justify-content:center;font-size:16px;flex-shrink:0;margin-top:1px}
.volume-warning-text{flex:1;font-size:11px;line-height:1.6;color:var(--t1)}
.volume-warning-text strong{font-weight:700;color:var(--t1);display:block;margin-bottom:3px}
.volume-warning-link{color:#d97706;text-decoration:none;font-weight:600}
.volume-warning-link:hover{text-decoration:underline}
...
<section class="pg on" id="pg-overview">
  ...
  <div class="info-boxes">
    <div class="info-box">
      <div style="display:flex;align-items:center;gap:10px">
        <div class="info-box-icon time"><i class="ti ti-clock"></i></div>
        <div>
          <div class="info-box-label">زمان باقی مانده</div>
          <div class="info-box-value" id="remaining-time">۲ روز ۳ ساعت</div>
          <div class="info-box-sub">برای کانفیگ‌های فعال</div>
        </div>
      </div>
    </div>
    <div class="info-box">
      <div style="display:flex;align-items:center;gap:10px">
        <div class="info-box-icon users"><i class="ti ti-users"></i></div>
        <div>
          <div class="info-box-label">تعداد یوزرها</div>
          <div class="info-box-value" id="user-count">—</div>
          <div class="info-box-sub">کاربرانِ فعال</div>
        </div>
      </div>
    </div>
  </div>
  <div class="volume-warning" id="volume-warning" style="display:none">
    <div class="volume-warning-icon"><i class="ti ti-alert-triangle"></i></div>
    <div class="volume-warning-text">
      <strong>🔥 دقت: حجم کانفیگت کم است!</strong>
      اگر می‌خوای همیشه متصل بمونی و بدون قطع‌شدگی کار کنی، می‌تونی تلگرام رو باز کنی و به آیدی 
      <a class="volume-warning-link" href="https://t.me/aria2773" target="_blank">@aria2773</a> 
      پیام بدی تا برات برنامه‌ای مختص بسازم و همیشه آنلاین بمونی! 💎
    </div>
  </div>
  <div class="vless-box">
    <div class="vl-header">
  ...
  <div class="dash-footer">
    <span class="df-text">سازنده پنل · X4G v9.2 · Railway</span>
    <a class="df-link" href="https://t.me/aria2773" target="_blank"><i class="ti ti-brand-telegram"></i> پشتیبانی</a>
...
function updateRemainingTime(){
  try{
    const links=allLinksList.filter(l=>l.active&&!l.expired);
    if(!links.length){document.getElementById('remaining-time').textContent='—';return}
    let minExpiry=null;
    links.forEach(l=>{
      if(l.expires_at){const exp=new Date(l.expires_at).getTime();if(!minExpiry||exp<minExpiry)minExpiry=exp}
    });
    if(!minExpiry){document.getElementById('remaining-time').textContent='نامحدود';return}
    const now=Date.now(),diff=Math.max(0,minExpiry-now);
    const days=Math.floor(diff/(1000*60*60*24)),hours=Math.floor((diff%(1000*60*60*24))/(1000*60*60)),mins=Math.floor((diff%(1000*60*60))/(1000*60));
    let txt='';
    if(days>0)txt+=toFa(days)+' روز ';
    if(hours>0||days>0)txt+=toFa(hours)+' ساعت ';
    if(mins>0||hours>0||days>0)txt+=toFa(mins)+' دقیقه';
    if(!txt)txt='کمتر از یک دقیقه';
    document.getElementById('remaining-time').textContent=txt.trim();
  }catch(e){console.error('Error updating remaining time:',e)}
}
function updateUserCount(){
  try{
    const links=allLinksList.filter(l=>l.active&&!l.expired);
    document.getElementById('user-count').textContent=toFa(links.length);
  }catch(e){console.error('Error updating user count:',e)}
}
function checkLowVolume(){
  try{
    ...
    for(const l of links){
      if(l.limit_bytes>0){const used=l.used_bytes||0,pct=used/l.limit_bytes*100;if(pct>85){hasLowVolume=true;break}}
    }
    const warningEl=document.getElementById('volume-warning');
    if(warningEl){warningEl.style.display=hasLowVolume?'flex':'none'}
  }catch(e){console.error('Error checking volume:',e)}
}
async function loadLinks(){
  try{
    ...
    updateRemainingTime();updateUserCount();checkLowVolume();
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  setInterval(()=>{
    updateRemainingTime();checkLowVolume();
  },60000);
});
</script>
</body></html>"""
...
.utxt{{font-size:10px;color:var(--t3);display:flex;justify-content:space-between}}
.cfg-extra{{display:flex;gap:7px;flex-wrap:wrap;margin-top:10px}}
.cfg-extra-item{{display:flex;align-items:center;gap:5px;font-size:10px;font-weight:700;color:var(--t2);background:var(--accent-d);border:1px solid var(--card-b);border-radius:9px;padding:5px 10px}}
.cfg-extra-item i{{color:var(--accent2);font-size:12px}}
.cfg-extra-item.warn{{color:var(--amber-t);background:var(--amber-bg);border-color:rgba(242,163,61,.25)}}
.cfg-extra-item.warn i{{color:var(--amber-t)}}
...
.lowvol-modal{{display:none;position:fixed;inset:0;background:rgba(0,0,0,.72);z-index:700;align-items:center;justify-content:center;backdrop-filter:blur(6px);padding:20px}}
.lowvol-modal.open{{display:flex}}
.lowvol-box{{background:var(--card);border:1px solid rgba(242,163,61,.3);border-radius:24px;padding:0;text-align:center;max-width:360px;width:100%;box-shadow:var(--shadow);overflow:hidden;position:relative}}
.lowvol-banner{{background:linear-gradient(150deg,rgba(242,163,61,.22),rgba(242,163,61,.02) 70%);padding:32px 26px 22px;position:relative}}
.lowvol-icon{{width:60px;height:60px;border-radius:18px;background:var(--amber-bg);border:1px solid rgba(242,163,61,.35);display:flex;align-items:center;justify-content:center;margin:0 auto 14px;font-size:28px}}
.lowvol-title{{font-size:16.5px;font-weight:800;color:var(--t1);margin-bottom:8px}}
.lowvol-text{{font-size:12.5px;color:var(--t2);line-height:1.9;padding:0 4px}}
.lowvol-body{{padding:20px 26px 26px}}
.lowvol-tg{{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:linear-gradient(135deg,#2FA3FF,#1E7CE0);color:#fff;border:none;border-radius:14px;padding:13px;font-family:inherit;font-size:13.5px;font-weight:800;cursor:pointer;text-decoration:none;margin-bottom:10px;box-shadow:0 8px 22px rgba(47,163,255,.3);transition:.18s}}
.lowvol-tg:hover{{transform:translateY(-1px)}}
.lowvol-close{{width:100%;background:transparent;border:none;color:var(--t3);font-family:inherit;font-size:11.5px;font-weight:600;cursor:pointer;padding:8px}}
.lowvol-close:hover{{color:var(--t2)}}
...
<div class="lowvol-modal" id="lowvol-modal" onclick="if(event.target===this)this.classList.remove('open')">
  <div class="lowvol-box" onclick="event.stopPropagation()">
    <div class="lowvol-banner">
      <div class="lowvol-icon">⚠️</div>
      <div class="lowvol-title">هی سلام 👋</div>
      <div class="lowvol-text">حجم کانفیگت داره تموم می‌شه 📉😬<br>اگه می‌خوای همیشه وصل بمونی و قطع نشی 🔌✅<br>همین الان به آیدی زیر توی تلگرام پیام بده 👇</div>
    </div>
    <div class="lowvol-body">
      <a class="lowvol-tg" href="https://t.me/aria2773" target="_blank" rel="noopener"><i class="ti ti-brand-telegram"></i> پیام به @aria2773 ✈️</a>
      <button class="lowvol-close" onclick="document.getElementById('lowvol-modal').classList.remove('open')">بعداً یادم بنداز 🙏</button>
    </div>
  </div>
</div>
<div class="wrap">
...
let savedPw='';
let lowVolShown=false;
const LOWVOL_THRESHOLD_BYTES = 1024*1024*1024; // ۱ گیگابایت

function checkLowVolume(d){{
  if(lowVolShown) return;
  const low = (d.links||[]).some(l => l.active && l.limit_bytes > 0 && (l.limit_bytes - l.used_bytes) <= LOWVOL_THRESHOLD_BYTES);
  if(low){{
    lowVolShown=true;
    document.getElementById('lowvol-modal').classList.add('open');
  }}
}}
...
function fmtB(b){{if(!b||b===0)return '0 B';if(b<1024)return b+' B';if(b<1024**2)return (b/1024).toFixed(1)+' KB';if(b<1024**3)return (b/1024**2).toFixed(2)+' MB';return (b/1024**3).toFixed(2)+' GB'}}
function toFa(n){{return String(n).replace(/\\d/g,d=>'۰۱۲۳۴۵۶۷۸۹'[d])}}
function fmtUsers(ipLimit,connections){{
  const conn = toFa(connections||0);
  if(!ipLimit || ipLimit===0) return conn+' کاربر (نامحدود)';
  return conn+' از '+toFa(ipLimit)+' کاربر';
}}
function fmtRemain(expiresAt){{
  if(!expiresAt) return {{text:'زمان نامحدود',warn:false}};
  const diff = new Date(expiresAt).getTime() - Date.now();
  if(diff<=0) return {{text:'منقضی شده',warn:true}};
  const days = Math.floor(diff/86400000), hours = Math.floor((diff%86400000)/3600000);
  if(days>0) return {{text:toFa(days)+' روز مانده',warn:days<=3}};
  const mins = Math.floor((diff%3600000)/60000);
  return {{text:toFa(hours)+' ساعت و '+toFa(mins)+' دقیقه مانده',warn:true}};
}}
  ...
  document.getElementById('root').innerHTML=`
    ...
    <div class="cfg-grid">
      ${{d.links.map((l, i) => {{
        ...
        const lim = l.limit_bytes === 0 ? '∞' : fmtB(l.limit_bytes);
        const usersInfo  = fmtUsers(l.ip_limit, l.connections);
        const remainInfo = fmtRemain(l.expires_at);
        return `
          <div class="cfg-card${{l.active ? '' : ' inactive'}}">
            <div class="cfg-top">
              ...
              <div class="cfg-extra">
                <span class="cfg-extra-item"><i class="ti ti-users"></i> ${{usersInfo}}</span>
                <span class="cfg-extra-item${{remainInfo.warn ? ' warn' : ''}}"><i class="ti ti-hourglass-low"></i> ${{remainInfo.text}}</span>
              </div>
            </div>

...
async def _reaper():
    # نکته‌ی مهم: قبلاً سشن‌هایی که tcp_open=True بودن از پاکسازی معاف بودن، که باعث
    # می‌شد بعد از هر reconnect کلاینت (تعویض وای‌فای/دیتا، پس‌زمینه رفتن اپ و…) سشن
    # قدیمی برای همیشه در connections بمونه («اتصالات ارواح») و شمارشِ کاربرِ آنلاین
    # عدد الکی و رو به رشد (مثلاً ۵۹ به‌جای ۱) نشون بده. حالا هر سشنی که مدت
    # SESSION_IDLE_TIMEOUT هیچ ترافیکی (آپلینک/دانلینک) نداشته پاک می‌شه، فارغ از
    # اینکه تونل TCPش باز بوده یا نه؛ سشن‌های واقعاً فعال چون last_seen مدام آپدیت
    # می‌شه امن می‌مونن.
    while True:
